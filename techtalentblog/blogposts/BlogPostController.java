...
public class BlogPostController {
	...
	public String index(Model model) {
		...
		List<BlogPosts> posts = new ArrayList<>();
		for(BlogPosts post: blogPostRepository.findAll()) {
			posts.add(post);
		}		
	...
	public String newBlog(Model model) {
		model.addAttribute("blogPost", new BlogPosts());
		return "blogpost/new";
	}
	
	
	@PostMapping(path="/blogposts")
	public String addNewBlogPost(BlogPosts blogPost, Model model) {
	...
	public String editPostWithId(@PathVariable Long id, Model model) {
		...
		// that id.
		Optional<BlogPosts> postBox=blogPostRepository.findById(id);
		if(postBox.isPresent()) {
			BlogPosts post = postBox.get();
	...
	public String updateExistingPost(@PathVariable Long id, BlogPosts blogPost, Model model) {
		Optional<BlogPosts> postBox = blogPostRepository.findById(id);
		if(postBox.isPresent()) {
			BlogPosts actualPost = postBox.get();
